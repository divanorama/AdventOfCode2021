...
--  let maxt = 100
  ...
  in if prelude.Natural.lessThan arrt.size nm then None Natural else
    ...
    let statet = { score : Optional Natural, map : arrt.repr, isBooming : arrbt.repr, numBooming : Natural, currT : Natural }
    let mInc = \(a : arrt.repr) -> \(ij : ijt) -> arrops.modify a (code ij) (\(curr : Optional Natural) -> Some (1 + (merge { None = 0, Some = id Natural} curr)))
    let step = \(s : statet) -> if prelude.Bool.not (prelude.Optional.null Natural s.score) then s else
      ...
      in if prelude.Natural.lessThan 0 newBooms
        then
          ...
          then if prelude.Natural.equal nm s.numBooming 
            then { score = Some s.currT, map = s.map, isBooming = s.isBooming, numBooming = s.numBooming, currT = s.currT}
            else
              ...
              in { score = s.score, map = newMap, isBooming = arrbops.empty, numBooming = 0, currT = s.currT }
    ...
    in (prelude.Natural.fold (maxt * 10) statet step { score = None Natural, map = map, isBooming = arrbops.empty, numBooming = 0, currT = 0 }).score

--let tex0 = assert : solve 200 ex0 === Some 6
--let tex1 = assert : solve 200 ex === Some 195
let tex = assert : solve 200 ex === Some 195
--in solve 2 ex0
in solve 10000 ./input.dhall
