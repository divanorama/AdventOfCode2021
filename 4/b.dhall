...
let isSome = \(t : Type) -> \(x : Optional t) -> merge {Some = \(v: t) -> True, None = False} x
let isNone = \(t : Type) -> \(x : Optional t) -> merge {Some = \(v: t) -> False, None = True} x
...
let solve = \(inp : inpt) ->
  ...
  let step = \(x : Natural) -> \(s : state) -> 
    let hands1 = map handt handt (act x) s.hands
    let scs = map handt (Optional Natural) (score x) hands1
    let hands2 : List handt = filter handt (\(h: handt) -> isNone Natural (score x h)) hands1
    let up = \(a : Optional Natural) -> \(acc : Optional Natural) -> 
      merge { Some = \(x: Natural) -> a , None = acc } a
    let bs = fold (Optional Natural) scs (Optional Natural) up s.won
    in {hands = hands2, won = bs}
  in (fold Natural (reverse Natural inp.picks) state step st0).won

let tex = assert : (solve ex) === Some 1924
