...
let solve = \(inp : inpt) ->
  ...
  let shoot = \(vx : Integer)  -> \(vy : Integer) ->
    let st = {x : Integer, y : Integer, vx : Integer, vy : Integer, res : Optional Bool}
    let step = \(s : st) -> 
      ...
      ] then {x = s.x, y = s.y, vx = s.vx, vy = s.vy, res = Some False} else
      if prelude.Bool.and [inx s.x, iny s.y] then {x = s.x, y = s.y, vx = s.vx, vy = s.vy, res = Some True}
      else {x = prelude.Integer.add s.x s.vx, y = prelude.Integer.add s.y s.vy, vx = if prelude.Integer.lessThan +0 s.vx then prelude.Integer.subtract +1 s.vx else s.vx, vy = prelude.Integer.subtract +1 s.vy, res = s.res }
    let r = prelude.Natural.fold (inp.ny.mx * 2 + 1) st step { x = +0, y = +0, vx = vx, vy = vy, res = None Bool }
    in merge {
      None = False,
      Some = prelude.Function.identity Bool
    } r.res
    
  let itery = \(s : Natural) -> \(vy : Integer) ->
    let iterx = \(s : Natural) -> \(vx : Integer) ->
      let cur = shoot vx vy
      in if cur then s + 1 else s
    in prelude.List.foldLeft Integer (ienum xr) Natural iterx s
  in prelude.List.foldLeft Integer (ienum yr) Natural itery 0
--  in ienum yr 
