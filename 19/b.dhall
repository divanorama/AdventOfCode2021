...
let at = {dots : ldott, scanner : dott }
let attempt = \(a : ldott) -> \(b : ldott) ->
  ...
  in merge {
    ...
    Some = \(x : freqt) -> 
      ...
      in Some { dots = prelude.List.map dott dott (add v) b, scanner = v }
...
let mdst = \(a : dott) -> \(b : dott) ->
  prelude.Natural.sum [ 
    prelude.Integer.abs (prelude.Integer.subtract a.x b.x),
    prelude.Integer.abs (prelude.Integer.subtract a.y b.y),
    prelude.Integer.abs (prelude.Integer.subtract a.z b.z),
  ]


let solve2 = \(l : ldott) ->
  let f = \(a : dott) -> prelude.List.map dott Natural (mdst a) l
  in prelude.Natural.listMax ( prelude.List.concatMap dott Natural f l )
...
let solve = \(inp : inpt) ->
  ...
  let st = { q : List elemt, saw : List Natural, scanners : ldott }
  let dfs = \(s : st) -> let rq = prelude.List.drop 1 elemt s.q
    in merge {
      None = s,
      Some = \(hd : elemt) -> if prelude.Natural.lessThanEqual n hd.minj then {q = rq, saw = s.saw, scanners = s.scanners} else
        if elem hd.minj s.saw then {q = [{i = hd.i, minj = hd.minj + 1, dots = hd.dots }] # rq, saw = s.saw, scanners = s.scanners} else
          let to = att ldott ([] : ldott) hd.minj inp
          let rots = prelude.List.map dotf ldott (\(f : dotf) -> prelude.List.map dott dott f to) dotfs
          let rotres = prelude.List.map ldott (Optional at) (attempt hd.dots) rots
          let oks = unopt at rotres
          in merge {
            None = {q = [{i = hd.i, minj = hd.minj + 1, dots = hd.dots }] # rq, saw = s.saw, scanners = s.scanners},
            Some = \(a : at) -> let tos = a.dots in {q = [{i = hd.minj, minj = 1, dots = tos}, {i = hd.i, minj = hd.minj + 1, dots = hd.dots }] # rq, saw = [hd.minj] # s.saw, scanners = [a.scanner] # s.scanners}
          } (prelude.List.head at oks)
        
    } (prelude.List.head elemt s.q) 
  let d0 = att ldott ([] : ldott) 0 inp
  in solve2 (prelude.Natural.fold (n * n) st dfs { q = [{i = 0, minj = 1, dots = d0}], saw = [0], scanners = [{x=+0,y=+0,z=+0}]}).scanners
