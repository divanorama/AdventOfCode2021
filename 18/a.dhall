...
let solve = \(inp : List (List e)) ->
  ...
  let f = \(acc : Optional numt) -> \(x : numt) ->
    merge {
      None = None numt,
      Some = \(acc : numt) -> add acc x
    } acc
  let hd = prelude.Optional.default numt ([] : numt) (prelude.List.head numt inp)
  let res = prelude.List.foldLeft numt (prelude.List.drop 1 numt inp) (Optional numt) f (Some hd)
  in merge {
    None = None Natural,
    Some = \(x : numt) -> Some (magn x)
  } res
