...
let m1 = \(v : Type) -> \(e : Type) -> { 
repr = {info : nodeinfo e, 
n1 : Optional (kvt v e), 
n2 : Optional (kvt v e), 
n3 : Optional (kvt v e),
n4 : Optional (kvt v e),
...
let n1 = prelude.List.head (kvt v e) l
let n2 = prelude.List.head (kvt v e) (prelude.List.drop 1 (kvt v e) l)
let n3 = prelude.List.head (kvt v e) (prelude.List.drop 2 (kvt v e) l)
let n4 = prelude.List.head (kvt v e) (prelude.List.drop 3 (kvt v e) l)
let n5 = prelude.List.head (kvt v e) (prelude.List.drop 4 (kvt v e) l)
let n6 = prelude.List.head (kvt v e) (prelude.List.drop 5 (kvt v e) l)
in {
...
let _m1ops = \(v : Type) -> \(e : Type) -> \(em : monot e) -> \(kv2e : Natural -> v -> e) ->
  ...
  let nodes = \(x : repr) -> prelude.List.unpackOptionals (kvt) [x.n1, x.n2, x.n3, x.n4, x.n5, x.n6]
  let split = \(x : repr) -> prelude.List.map (List kvt) repr (mkm1 v e em) [prelude.List.take n2 kvt (nodes x), prelude.List.drop n2 kvt (nodes x)]
  let withgrow = \(res : repr) ->
    if prelude.Natural.lessThan n (prelude.List.length kvt (nodes res)) then (growt repr).extend (split res)
    else (growt repr).fits res
  in {
    getinfo = \(m : repr) -> m.info,
    get = 
      let f = \(k : Natural) -> \(x : kvt) -> prelude.Natural.equal k x.key
      in \(m : repr) -> \(k : Natural) -> prelude.List.head v (prelude.List.map kvt v getv (prelude.List.filter kvt (f k) (nodes m))),
    upsert =
      let lek = \(k : Natural) -> \(x : kvt) -> prelude.Natural.lessThan (x.key) k
      in \(m : repr) -> \(k : Natural) -> \(f : Optional v -> v) ->
        let pt = prelude.List.partition kvt (lek k) (nodes m)
        let a = pt.true
        let b = pt.false
        let vb = merge {
          None = {_1 = None v, _2 = b},
...
--        let res = { nodes = a # [z] # b, info = {min = prelude.Natural.min z.key m.info.min, max = prelude.Natural.max z.key m.info.max, extra = em.cmb z.extra m.info.extra}}
        in withgrow res,
    join = \(a : repr) -> \(b : repr) ->
--      let res = { nodes = (nodes a) # (nodes b), info = combinfos e em a.info b.info }        
      let res = mkm1 v e em ((nodes a) # (nodes b))
      in withgrow res,
...
let m2 = \(m : mapt) -> { repr = {info : nodeinfo m.e, 
n1 : Optional m.repr, 
n2 : Optional m.repr, 
n3 : Optional m.repr,
n4 : Optional m.repr,
...
let n1 = prelude.List.head m.repr l
let n2 = prelude.List.head m.repr (prelude.List.drop 1 m.repr l)
let n3 = prelude.List.head m.repr (prelude.List.drop 2 m.repr l)
let n4 = prelude.List.head m.repr (prelude.List.drop 3 m.repr l)
let n5 = prelude.List.head m.repr (prelude.List.drop 4 m.repr l)
let n6 = prelude.List.head m.repr (prelude.List.drop 5 m.repr l)
in {  info = joininfos m.e em (prelude.List.map m.repr (nodeinfo m.e) ops.getinfo l),
--  nodes = l,
n1 = n1, n2 = n2, n3 = n3, n4 = n4, n5 = n5, n6 = n6,
} : (m2 m).repr
...
let _m2ops = \(prev : mapt) -> \(em : monot prev.e) -> \(ops : _mapops prev) -> 
  ...
  let nodes = \(x : repr) -> prelude.List.unpackOptionals prev.repr [x.n1, x.n2, x.n3, x.n4, x.n5, x.n6]
  let maybegrow = \(res : repr) ->
    if prelude.Natural.lessThan n (prelude.List.length prev.repr (nodes res)) then (growt repr).extend (split (nodes res))
    else (growt repr).fits res
  in {
    getinfo = \(m : repr) -> m.info,
    get = 
      ...
      in \(m : repr) -> \(k : Natural) -> unopt (prelude.List.head ov (prelude.List.map prev.repr ov (\(m : prev.repr) -> ops.get m k) (prelude.List.filter prev.repr (f k) (nodes m)))),
    upsert =
      let pf = \(k : Natural) -> \(x : prev.repr) -> prelude.Natural.lessThanEqual (ops.getinfo x).min k
      in \(m : repr) -> \(k : Natural) -> \(f : ov -> v) -> 
        let pt = prelude.List.partition prev.repr (pf k) (nodes m)
        let a = pt.true
        let b = pt.false
        let acb = merge {
...
--        let res = {nodes = a # c # b, info = joininfos (prev.e) em (prelude.List.map prev.repr (nodeinfo prev.e) ops.getinfo (a # c # b))}
        let res = mkm2 prev em ops (a # c # b)
        in maybegrow res
      ,
    join = \(a : repr) -> \(b : repr) ->
--      let res = {nodes = a.nodes # b.nodes, info = combinfos prev.e em a.info b.info}
      let res = mkm2 prev em ops ((nodes a) # (nodes b))
      in maybegrow res
      ,
