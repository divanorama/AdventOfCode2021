...
let inpt = { start : Natural, end : Natural, edges : List (List Natural) }
...
let solve = \(inp : inpt) ->
  let n = 1 + (prelude.List.foldLeft Natural (prelude.List.concat Natural inp.edges) Natural prelude.Natural.max 0)
  let pn = pw2 n
  ...
  in if prelude.Natural.lessThan earrt.size (n * n) then outt.smalle {want = n * n, have = earrt.size} else --None Natural else
    ...
    in if prelude.Natural.lessThan dparrt.size (n * pn) then outt.smalldp {want = n * pn, have = dparrt.size} else
      ...
      let es = prelude.List.concatMap (List Natural) et pairs inp.edges
